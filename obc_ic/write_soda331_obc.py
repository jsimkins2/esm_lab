# gridtools python env https://github.com/ESMG/gridtools/blob/main/conda/gridTools.yml
# had to install glob (known as glob2 on conda-forge) and bottleneck via conda-forge channel too
# note that I had to downgrade xarray to 0.18.2 because I was receiving an error "Using a DataArray object to construct a variable is ambiguous"
import numpy as np
from os import path
import warnings
    ...
    for y in years:
        ...
        if y == years[0]:
            if y == 1979:
                ...
                sodafiles = [path.join(soda_root, soda_version, str(y), f'{soda_prefix}_5dy_ocean_reg_1980_01_03.nc')]
            else:
                # Just late December of first year
                sodafiles = sorted(glob(path.join(soda_root, soda_version, str(y), f'{soda_prefix}_5dy_ocean_reg_{y}_12*')))[-1]
        elif y == years[-1]:
            # Just early January of end year
            sodafiles = sorted(glob(path.join(soda_root, soda_version, str(y), f'{soda_prefix}_5dy_ocean_reg_{y}_01_0*')))[0]
        else:
            sodafiles = sorted(glob(path.join(soda_root, soda_version, str(y), f'{soda_prefix}_5dy_ocean_reg_{y}*')))
...
def main():
    ...
    soda_root = '/Volumes/P1/Data/SODA/'
    soda_version = 'SODA_3.3.1' # SODA_3.3.1 is latest on Proteus, but 3.12.2 is on Cheyenne
    soda_prefix = 'soda3.3.1'
    outdir = '/Users/james/soda_output/SODA_3.3.1/'

    hgrid = xarray.open_dataset('/Users/james/Documents/nwaGrid/nwa_ocean_hgrid.nc')
    segments = [
